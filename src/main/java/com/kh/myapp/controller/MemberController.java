...
public class MemberController {
	...
	// @Valid : 검증하는 것. 예전에 JSP, 자바스크립트로 유효성 체크했는데 얘로 back단에서 유효성체크 가능해짐.
	// (hibernate.jar 3개 추가했음)
	// 셧다운방식인 hibernate..
	...
	@RequestMapping(value = "/findPasswdOK")
	public String findPasswdOK(@RequestParam("id") String id, @RequestParam("name") String name,
			@RequestParam("phone") String phone, Model model) {

		model.addAttribute("passwd", memberService.findPasswd(id, name, phone));
		return "/member/findPasswd";
	}
