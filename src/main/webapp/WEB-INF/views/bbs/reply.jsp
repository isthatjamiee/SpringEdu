		...
		$.ajax({
			...
			success: function(data){
				$.each(data.item, function(idx,rec){
					...
					str	+=	"<p class='pull-right' align='right'><small>" + rec.rcdate + "</small></p>&nbsp;"
						 	+		"<span>[" + rec.rname 		+ "]</span>"								 
 							+		"<span>&nbsp;&nbsp;</span>"
						 	+ 	"<span id='rcontent'>" + rec.rcontent 	+ "</span>"
							+		"<span>&nbsp;&nbsp;&nbsp;</span>";						 								 	
					if(rec.rid == login_id){
						str	+= 	"<button id='b0' class='btn btn-primary btn-circle'>댓글</button>&nbsp;"						 	
						 	+ 	"<button id='b1' class='btn btn-info btn-circle'>수정</button>&nbsp;"
						 	+ 	"<button id='b2' class='btn btn-success btn-circle'>삭제</button>&nbsp;"
 						 	+ 	"<button type='button' id='b3' class='btn btn-danger btn-circle'>"
						 	+		"<i class='glyphicon glyphicon-heart'>&nbsp;" + rec.rgood + "</i></button>&nbsp;" 	 						 	
						 	+ 	"<button type='button' id='b4' class='btn btn-warning btn-circle'>"
						 	+		"<i class='glyphicon glyphicon-thumbs-down'>&nbsp;" + rec.rbad + "</i></button>&nbsp;"						 	
						 	+ 	"</li></span>";
					}else{ 
...
<body>
	<div class="form-group" id="reReplyDiv" style="font-family:휴먼편지체 보통;">
		<span class="title-dialog"></span>번 글에 댓글달기
		<div>&nbsp;
			<textarea class="form-control" id="orgContent" cols="50" rows="2" readOnly="readonly"></textarea>
			&nbsp;
			<textarea class="form-control" id="reReplyContent" cols="50" rows="3"></textarea>
		</div>&nbsp;
		<div align="right">
			<button id="reReplyBtn" class="btn btn-info">댓글</button>
			<button id="reReplyDiv_closeBtn" class="btn btn-info">닫기</button>
		</div>
	</div>
	...
	<div class="form-group" id="modifyDiv">
		...
		<div align="right">
			<button id="reModifyBtn" class="btn btn-info">수정</button>&nbsp;
			<button id="modifyDiv_closeBtn" class="btn btn-info">닫기</button>
		</div>
	</div>
	...
	<div class="table-responsive-sm">
		<table class="table table-sm">
			<tr class="table-table">
				...
				<td><textarea name="" class="form-control" id="replyContent"
						autofocus="" placeholder="Write in your wall" cols="120" rows="3"></textarea></td>
				<td class="align-middle"><button style=height:70px id="replyBtn" type="submit"
						class="[ btn btn-success ]">댓글작성</button></td>
			</tr>

		</table>
	</div>

	<h4 align="center">Comment List</h4>
